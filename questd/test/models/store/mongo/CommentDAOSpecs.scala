...
class CommentDAOSpecs extends Specification
  ...
  "Mongo Comment DAO" should {
    ...
    "Get all comments" in new WithApplication(appWithTestDatabase) {
      ...
      val authorIds = db.comment.allWithParams(objectIds = List(comments(1).info.commentedObjectId)).toList
      authorIds.map(_.id) must beEqualTo(List(comments(1).id))

      val authorIdsSkip = db.comment.allWithParams(objectIds = comments.map(_.info.commentedObjectId).take(2), skip = 1).toList
