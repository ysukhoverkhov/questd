...
//      statusVip.map(_.id) must beEqualTo(List(comments(0).id))
//
//      val ids = db.battle.allWithParams(ids = List("b1", "b2")).toList
//      ids.map(_.id).size must beEqualTo(2)
//      ids.map(_.id).sorted must beEqualTo(List(comments(0).id, comments(1).id).sorted)
//
//      val culture = db.battle.allWithParams(cultureId = Some(comments(2).cultureId)).toList
//      culture.map(_.id).size must beEqualTo(1)
//      culture.map(_.id) must beEqualTo(List(comments(2).id))
//
//      val excludingIds = db.battle.allWithParams(idsExclude = List("b1", "b2")).toList
//      excludingIds.map(_.id).size must beEqualTo(1)
//      excludingIds.map(_.id) must beEqualTo(List(comments(2).id))
//
...
//      excludingAuthorIds.map(_.id).sorted must beEqualTo(List(comments(1).id).sorted)
//
//      val solutionIds = db.battle.allWithParams(solutionIds = List("s1_id")).toList
//      solutionIds.map(_.id).size must beEqualTo(3)
//      solutionIds.map(_.id).sorted must beEqualTo(comments.map(_.id).sorted)
    }
//
